...
(defvar dtd-use-after-change-functions-flag t
  "*Non-nil enables use of after-change functions")
...
(defun dtd-toggle-debug-on-error ()
  ...
  (let ((fill-column dtd-mdc-indent-column)
	...
	(save-excursion
	  (font-lock-fontify-region
	   (dtd-font-lock-region-point-min)
	   (dtd-font-lock-region-point-max))))
    ;; Add to the list of element type names that has already been seen
    (add-to-list 'dtd-declared-element-type-names element-tag)
    ;; Work out whether it's safe to remove element-tag from the list of
    ;; element type names that's been seen referenced in content models
    ;; and parameter entity declarations
    ;;
    ;; Before working anything out, need to remove element-type-name
...
;;  (message "%s" content-spec)
  (if (not (or (string-equal content-spec "ANY")
	       (string-equal content-spec "CDATA")
	       (string-equal content-spec "EMPTY")
	       (string-equal content-spec "RCDATA")))
      (let ((match-index nil)
	    (token-list nil))
	(save-match-data
...
;;(defun dtd-update-element-lists-with-element
;;  "Analyse the elements definition as if it had just been declared."
;;  (if (looking-at "\\(<!ELEMENT\\)\\(\\s-+\\)\\sw+")
;;	  (progn
...
(defun dtd-defined-in-comment (element-tag other-module)
  "Insert an \"ELEMENT-TAG  is defined in OTHER-MODULE\" comment.

ELEMENT-TAG is the element type name as it appears in start- and
end-tags.  OTHER-MODULE is the name of the module in which the element
is defined.

An example inserted \"defined in\" comment is as follows:

<!--       ELEMENT-TAG  is defined in OTHER-MODULE                 -->
"
  (interactive
   ;; Hackery and fakery
   (let ((element-tag nil)
  ...
  (insert (format "is defined in %s" other-module))
  (indent-to dtd-comment-max-column)
  (insert dtd-comment-end)
  (insert "\n")
  ;; Add to the list of element type names that we've already seen
  (add-to-list 'dtd-declared-element-type-names element-tag)
  ;; Remove element-type-name from the start of
  ;; dtd-referenced-element-type-names (since that's the minibuffer
...
MDC = Markup Declaration Close"
  ...
  (save-excursion
    (let ((current-point (point))
	  (mdc-column 0)
	  (mdc dtd-sgml-mdc))
      ;; Only insert a mdc if looking at an mdo or if there's a previous mdo
	  ...
	  (progn
		...
		;; Delete to the end of line so we can then insert our mdc
	       ...
	       (insert mdc)))
	     ;; Another special case: just comment start and end
	     ((looking-at "\\(\\s-*<!--\\)\\s-+-->$")
...
(if dtd-mode-map
  ...
  (define-key dtd-mode-map [(control m)]	  'dtd-electric-return)
...
(defun dtd-electric-return ()
  "What do do for a newline in dtd mode"
  (interactive)
  ...
  (if (= (char-before) ?>)
    ...
    (progn
	 ...
	 (let*
		 ...
		 (re-search-backward "^<!")
		 (cond
		  ((looking-at "<!ELEMENT") 'element)
		  ((looking-at "<!ATTLIST") 'attlist)
		  ((looking-at "<!--") 'comment)
		  ((looking-at "<!ENTITY\\s-+%") 'parameter-entity)
		  ((looking-at "<!ENTITY") 'entity)
		   ...
		   ('element
		    (cond
		     ((looking-at "<!ELEMENT\\s-*$")
		      (message "ELEMENT")
		      (1- dtd-element-name-column))
		     ((looking-at "<!ELEMENT\\s-+\\sw+\\s-*$")
		      (message "ELEMENT name")
		      (1- (if dtd-xml-flag
			      dtd-xml-element-content-spec-start-column
			    dtd-element-tag-omission-column)))
		     ((looking-at
		       "<!ELEMENT\\s-+\\sw+\\s-+[-oO]\\s-+[-oO]\\s-*$")
		      (1- dtd-element-content-spec-start-column))
		     ((looking-at
		       "<!ELEMENT\\s-+\\sw+\\s-+[-oO]\\s-+[-oO]\\s-+\\S+")
		      (message "ELEMENT name")
		      dtd-element-content-spec-start-column)
		     ((looking-at "<!ELEMENT\\s-+\\sw+\\s-+[^-oO]")
		      (message "ELEMENT name xml")
		      (1-
		       dtd-xml-element-content-spec-continuation-column))
		     ((looking-at "\\s-+[-oO]\\s-+[-oO]\\s-+(")
		      (message "SGML element continuation")
		      (1- dtd-element-content-spec-continuation-column))
		     ((looking-at "\\(\\s-+\\)\\S-+")
		      (goto-char (match-end 1))
		      (current-column))
		    ...
		    ('attlist
		     (cond
		      ((looking-at "<!ATTLIST$")
		       (message "attlist")
		       (1- dtd-element-name-column))
		      ((looking-at "<!ATTLIST\\s-+\\sw+")
		       (message "attlist element name")
		       (1- dtd-attribute-name-column))
		      ((looking-at
			"\\(\\s-+\\(\\sw+\\s-+\\)?NOTATION\\s-+(\\)")
		       (if (save-excursion
			     (end-of-line)
			     (= (char-before) ?\)))
			   (progn
			     (message "Finished NOTATION list")
			     (1- dtd-attribute-default-column))
			 (progn
			   (message "Unfinished NOTATION list")
			   (- (match-end 1) (match-beginning 1)))))
		      ((looking-at "[^(]*([^)]*)\\s-+$")
		       (message "Finished group")
		       (1- dtd-attribute-default-column))
		      ((looking-at "\\(\\s-+\\)\\sw+\\s-*")
		       (message "attribute list, single token")
		       (end-of-line)
		       (delete-horizontal-space)
		       (cond
			((or
			  (= (char-before) ?\))
			  (= (char-before) ?\"))
			 (message "attribute list, after ) or \"")
			 (1- dtd-attribute-default-column))
			((<= (abs (- (- (match-end 1)
					(match-beginning 1))
				     dtd-attribute-name-column))
			     2)
			 (message "After attribute name")
			 (1- dtd-comment-start-column))
			(t
			 (goto-char (match-end 1))
			 (current-column))))
		      ((looking-at "\\(\\s-+\\)(")
		       (message "Start of group")
		       (goto-char (match-end 1))
		       (1+ (current-column)))
		      ((looking-at "\\(\\s-+\\)\\sw")
		       (if (<= (abs (- (length (- (match-end 1)
						  (match-beginning 1)))
				       dtd-attribute-name-column))
			       2)
			   (1- dtd-comment-start-column)
			...
			"<!ENTITY\\s-+%\\s-+\\sw+\\s-+\\(PUBLIC\\|SYSTEM\\)$")
		       (message "PUBLIC or SYSTEM")
		       0)
		      ((looking-at "<!ENTITY\\s-+%\\s-+\\sw+\\s-+\"")
		       (message "Internal PE continuation")
		       dtd-entity-entity-value-continuation-column)
		      ((looking-at "<!ENTITY\\s-+%\\s-+\\sw")
		       (message "Internal PE")
		       (1- dtd-entity-entity-value-start-column))
		     ((looking-at "\\(\\s-+\\)\\(\\sw\\|(\\)+")
		       dtd-entity-entity-value-continuation-column)
...
;;		   (looking-at "<!ATTLIST"))
;;		 (progn
;;		   (message "inside ATTLIST")
;;		   (end-of-line)
;;		   (backward-char)
;;		   (if (looking-at ")")
;;		       (1- dtd-attribute-default-column)))
;;	       (progn
;;		 (message "also inside ATTLIST")
;;		 ;;	       (end-of-line)
;;		 (if (looking-at "\\(\\s-+\\)\\(\\sw\\|(\\)+")
;;		     (goto-char (match-end 1)))
;;		 (current-column))))
...
;;(defun dtd-recalc-mdc-after-change (begin end length)
;;  "Recalculate the position of the mdc after a change"
;;  (if dtd-use-after-change-functions-flag
;;      (save-match-data
;;	(save-excursion
;;	  (goto-char end)
;;	  (end-of-line)
;;	  (if (and (= (char-before) ?>)
;;		   (or (= (char-before (- (point) 2)) ?-)
;;		       (= (char-before (- (point) 2)) ?\ )))
;;	      (dtd-insert-mdc))))))
...
\\{dtd-mode-map}"
  ...
  (setq after-change-functions '(dtd-recalc-mdc-after-change))
